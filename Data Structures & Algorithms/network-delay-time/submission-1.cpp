...
public:
    int networkDelayTime(vector<vector<int>>& times, int n, int k) {
        ...
        // In Dijkstras, once a node is finalized (popped from the min-heap), its shortest distance is determined.
        // Processing the same node again wastes time and can cause issues in some implementations.
        // Always skip nodes that have already been visited or whose current distance exceeds the known shortest.

...
public:
    int ladderLength(string beginWord, string endWord, vector<string>& wordList) {
        ...
        // If the resulting word exists in our word set, it is a change.
        // We chase down our result accordingly, one letter at a time. 
        while (!q.empty())
        {
            result++;
            int len = q.size();
            for (int i = 0; i < len; ++i)
            {
                ...
                q.pop();
                // If we ended up at the word, we're done.

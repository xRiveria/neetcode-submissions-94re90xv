...
public:
    /*
        1. Throw out smaller numbers behind the new one.
        2. Add the new number's index.
        3. Throw out old numbers that are outside the window.
        4. When window size == k → record the maximum (q.front()).
    */
    vector<int> maxSlidingWindow(vector<int>& nums, int k) {
       ...
       int l = 0, r = 0;
        ...
        // Dequeue tracks the maximum at all times.
        // Value at the front is the biggest always for the current window.
       while (r < n)
       {
        ...
        // This ensures that our maximum element is always within the window.
        if (l > q.front())
        {
            q.pop_front();
        }

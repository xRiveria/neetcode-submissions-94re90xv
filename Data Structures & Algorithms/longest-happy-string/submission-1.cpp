...
public:
    string longestDiverseString(int a, int b, int c) {
        ...
        while (!values.empty())
        {
            ...
            // We cannot insert first, and then check later. We must always prevent insertion from happening unless allowed!
            ...
            if (happyString.length() >= 2 && happyString[happyString.size() - 1] == topValue.second && happyString[happyString.size() - 2] == topValue.second)
            {
                if (values.empty())
                {
                    break; // There is no alternative. We're done. Doing anything else would make our string unhappy.
                }
